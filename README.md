...
直接运行 `vlp_wknn_astar_obstacles.m`。首次运行定位导航脚本时，建议保留
默认参数，以便快速生成指纹库、定位误差结果和规划路径。
运行完成后，四组结果图会自动保存至 `results/warehouse_navigation/`。

## 仿真场景

定位导航示例使用 `30 m × 20 m × 8 m` 仓库空间、20 个顶部 LED、
`0.5 m` 栅格分辨率及多排仓储货架。系统对比不同 K 值下的 WKNN 定位结果，
并将估计位置作为 A* 路径规划起点。为保证大型场景可直接运行，默认使用
LOS 信道构建指纹库；可在脚本中通过 `enable_NLOS` 开启一次反射 NLOS 模型。
